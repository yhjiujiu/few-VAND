...
def load(name: str, device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu", design_details = None, jit: bool = False, download_root: str = None):
    ...
    if name in _MODELS:
        ...
        model_path = _download(_MODELS[name], download_root or os.path.expanduser("/home/yinhong/.cache/clip"))
...
def get_similarity_map(sm, shape):
    ...
    #print("sm0: ",sm.size()) [8, 2, 37, 37]
    sm = torch.nn.functional.interpolate(sm, shape, mode='bilinear')
    #print("sm: ",sm.size()) [8, 2, 518, 518]
...
def compute_similarity(image_features, text_features, t=2):
    ...
    # 对应元素相乘
    #print("feats: ",b, n_t, n_i, c,feats.size())  
    #  8 2 1370 768,  [8, 1370, 2, 768]
    similarity = feats.sum(-1) # 求和
    return (similarity/0.07).softmax(-1), prob_1 #[8, 1370, 2]
